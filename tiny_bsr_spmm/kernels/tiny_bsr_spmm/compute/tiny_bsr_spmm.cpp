...
void kernel_main() {
    constexpr std::uint32_t max_kt_dim = get_compile_time_arg_val(0);
    const std::uint32_t num_tiles_this_core = get_arg_val<std::uint32_t>(0);

    _llk_unpack_sparse_8x16_hw_configure_<is_fp32_dest_acc_en>(bf16_format, bf16_format, 256, 512);
    _llk_unpack_sparse_8x16_init_();

    for (std::uint32_t i = 0; i < num_tiles_this_core; ++i) {
