...
#include "tiny_bsr_spmm/tiny_bsr_spmm_snf_program_factory.hpp"
...
TinyBsrSpmmSnfProgramFactory::cached_program_t TinyBsrSpmmSnfProgramFactory::create(
    ...
    TT_FATAL(M % 8 == 0, "tiny_bsr_spmm_snf: M must be a multiple of 8, got {}", M);
    TT_FATAL(N % 16 == 0, "tiny_bsr_spmm_snf: N must be a multiple of 16, got {}", N);
    TT_FATAL(max_kt_dim >= 1, "tiny_bsr_spmm_snf: max_kt_dim must be >= 1");
    TT_FATAL(
        a_faces.logical_shape() ==
            ttnn::Shape({static_cast<int>(num_output_rows * num_output_cols * max_kt_dim), 8, 16}),
        "tiny_bsr_spmm_snf: a_faces shape mismatch");
    TT_FATAL(
        b_tiles.logical_shape() ==
            ttnn::Shape({static_cast<int>(num_output_rows * num_output_cols * max_kt_dim), 16, 16}),
        "tiny_bsr_spmm_snf: b_tiles shape mismatch");
    ...
    const std::vector<std::uint32_t> in0_compile_args = {
        ...
        num_cores_x,
        num_cores_y};
    const std::vector<std::uint32_t> in1_compile_args = {
        b_is_dram ? 1U : 0U, max_kt_dim, num_output_rows, num_output_cols, num_cores_x, num_cores_y};
    ...
    const auto in1_id = CreateKernel(
        program,
        kernel_dir + "dataflow/reader_tiny_bsr_spmm_snf.cpp",
        cores,
        DataMovementConfig{
    ...
    const std::uint32_t sem_sender = CreateSemaphore(program, cores, 0);
    const std::uint32_t sem_receiver = CreateSemaphore(program, cores, 0);
    ...
    for (std::uint32_t cy = 0; cy < num_cores_y; ++cy) {
        for (std::uint32_t cx = 0; cx < num_cores_x; ++cx) {
            ...
            const std::vector<std::uint32_t> in0_runtime_args = {
                ...
                is_injector ? 1U : 0U,
                is_sink ? 1U : 0U};
            const std::vector<std::uint32_t> in1_runtime_args = {
                static_cast<std::uint32_t>(b_tiles_buffer->address()), cx, cy};
            const std::vector<std::uint32_t> compute_runtime_args = {num_tiles_this_core, cx, cy};
    ...
    return {std::move(program), {in0_id, in1_id, compute_id, sem_sender, sem_receiver, num_cores_x, num_cores_y}};
}

void TinyBsrSpmmSnfProgramFactory::override_runtime_arguments(
    ...
    for (std::uint32_t cy = 0; cy < shared.num_cores_y; ++cy) {
        for (std::uint32_t cx = 0; cx < shared.num_cores_x; ++cx) {
            ...
            const std::vector<std::uint32_t> in0_runtime_args = {
                ...
                is_sink ? 1U : 0U};
            const std::vector<std::uint32_t> in1_runtime_args = {
                static_cast<std::uint32_t>(tensor_args.b_tiles.buffer()->address()), cx, cy};
            const std::vector<std::uint32_t> compute_runtime_args = {num_tiles_this_core, cx, cy};
